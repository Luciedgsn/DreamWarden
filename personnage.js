...
export class Personnage {
    constructor(scene, position = new BABYLON.Vector3(0, 1, 0)) {
        ...
        this.isDead = false; // Indicateur pour savoir si le joueur est mort
    ...
    enableMovement() {
        ...
        this.scene.onBeforeRenderObservable.add(() => {
            if (this.health <= 0 && !this.isDead) {
                this.die(); // Appeler la méthode de mort si les PV atteignent 0
            }
        });
    ...
    castFireball() {
        ...
        fireball.position.y += 1; // Ajuster la position pour qu'elle soit au-dessus du personnage
    ...
    takeDamage(amount) {
        if (this.isDead) return; // Ne rien faire si le joueur est déjà mort
        this.health -= amount;
    ...
    restartScene() {
        console.log("Relance de la scène...");
